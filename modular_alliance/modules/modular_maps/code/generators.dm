/datum/map_generator/hetto_generator
	var/name = "Hetto Generator"
	///Weighted list of the types that spawns if the turf is open
	var/open_turf_types = list(/turf/open/floor/plating = 1, /turf/open/floor/grass = 1)
	...
	var/list/flora_spawn_list = list(/obj/structure/flora/bush/large/style_random = 2, /obj/structure/flora/bush/jungle/b/style_random = 2, \
		/obj/structure/flora/bush/sunny/style_random = 2, /obj/structure/flora/bush/jungle/c/style_random = 2,
	)
	///Weighted list of extra features that can spawn in the area, such as geysers.
	var/list/feature_spawn_list = list(/obj/structure/spacevine = 2, /obj/structure/spacevine = 8)
...
/datum/map_generator/hetto_generator/New()
	. = ..()
	if(!mob_spawn_list)
...
/datum/map_generator/hetto_generator/generate_terrain(list/turfs)
	. = ..()
	...
	for(var/i in turfs) //Go through all the turfs and generate them
		var/turf/gen_turf = i
		if(!ispath(gen_turf.type,/turf/open/genturf/alternative))
			continue
		...
		if(!closed)//Open turfs have some special behavior related to spawning flora and mobs.
			...
			if(flora_spawn_list && prob(flora_spawn_chance))
				...
				if(!(A.area_flags & FLORA_ALLOWED))
					can_spawn = FALSE
				if(can_spawn)
					if(ispath(new_turf.type,/turf/open/floor/grass))
