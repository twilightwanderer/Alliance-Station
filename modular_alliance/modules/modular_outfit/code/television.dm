...
/obj/item/device/modular_alliance/pocket_tvcamera/Initialize(mapload)
	. = ..()

/obj/item/device/modular_alliance/pocket_tvcamera/examine()
	. = ..()
...
/obj/item/device/modular_alliance/pocket_tvcamera/attack_self(mob/user)
	. = ..()
	if(check_id(user))
		if(!on)
			camera_on(user)
...
/obj/item/device/modular_alliance/pocket_tvcamera/proc/camera_on(mob/user)
	if(tv_camera == null)
...
/obj/item/device/modular_alliance/pocket_tvcamera/proc/camera_off(mob/user)
	if(tv_camera != null)
		tv_camera.status = 0
	if(internal_radio != null)
...
/obj/item/device/modular_alliance/pocket_tvcamera/proc/check_id(mob/living/user)
...
/obj/item/device/modular_alliance/pocket_tvcamera/CtrlClick(mob/user)
	. = ..()
	if(check_id(user))
		switch (sensitivity)
			if(0)
...
/obj/item/device/modular_alliance/pocket_tvcamera/equipped(mob/equipper, slot)
	. = ..()
	if(ishuman(equipper))
...
/obj/item/device/modular_alliance/pocket_tvcamera/dropped(mob/user)
	. = ..()
...
/obj/item/device/modular_alliance/pocket_tvcamera/proc/do_camera_update(oldLoc)
	if(!QDELETED(tv_camera) && oldLoc != get_turf(loc))
		GLOB.cameranet.updatePortableCamera(tv_camera)
	updating = FALSE

#define CAMERA_BUFFER 10
/obj/item/device/modular_alliance/pocket_tvcamera/proc/update_camera_location(oldLoc)
	if(!oldLoc)
		oldLoc = get_turf(loc)
...
/obj/item/device/modular_alliance/pocket_tvcamera/proc/in_inventory(mob/user)
