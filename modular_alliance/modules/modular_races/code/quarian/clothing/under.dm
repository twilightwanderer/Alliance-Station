...
/obj/item/clothing/under/modular_alliance/quarian/pickup(mob/living/carbon/human/user)
	. = ..()
...
/obj/item/clothing/under/modular_alliance/quarian/dropped(mob/living/carbon/human/user, silent = FALSE)
	. = ..()
	if(is_species(user, /datum/species/quarian))
		if(!check_tightness_quarian(user))
			to_chat(user, span_warning("Breach of tightness of the suit!"))
...
/obj/item/clothing/under/modular_alliance/quarian/equipped(mob/living/carbon/human/user, slot, initial = FALSE)
	. = ..()
